 ...
 * @constructor
 */
const debug = Debug("eventstore:eventdispatcher");

export class EventDispatcher {
  public undispatchedEventsQueue: any[];
  public isRunning: boolean = false;
  constructor(public publisher: any, public store: any) {
   ...
   * @param events
   */
  addUndispatchedEvents(events: Event[]) {
    events.forEach(function (event) {
      this.undispatchedEventsQueue.push(event);
    });
    trigger(this);
  }
  ...
  start(callback: Function) {
    if (typeof this.publisher !== "function") {
      const pubErrMsg = "publisher not injected!";
      debug(pubErrMsg);
    ...
    if (
      ...
      const storeErrMsg = "store not injected!";
      debug(storeErrMsg);
    ...
    this.store.getUndispatchedEvents(function (err, events: Event[]) {
      if (err) {
        debug(err);
 ...
 */
function trigger(dispatcher: EventDispatcher) {
  const queue = dispatcher.undispatchedEventsQueue || [];
